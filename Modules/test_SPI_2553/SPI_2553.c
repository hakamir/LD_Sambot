 ...
 *
 * MSP430G2553 - SPI/MASTER 3 Wires
 *
...
void init_USCI( void )
{
    // Waste Time for waiting Slave SYNC
    __delay_cycles(250);

    UCB0CTL0 = 0;
    UCB0CTL1 = (0 + UCSWRST*1 ); // SOFTWARE RESET - mode configuration
    ...
    UCB0CTL0 |= ( UCSYNC | UCMST | UCMODE_0 );   // 3-pin, 8-bit
    UCB0CTL0 &= ~(UCDORM | UCCKPH  | UCCKPL | UC7BIT | UCMSB | UCMM );
    UCB0CTL1 |= UCSSEL_2;                                // SMCLK

    UCB0BR0 = 0x0A;     // divide SMCLK by 100 = 100kHz
    UCB0BR1 = 0x00;
